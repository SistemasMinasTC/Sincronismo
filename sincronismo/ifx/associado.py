...
def convert(conn_ifx, conn_sql, linha_log):
    ...
    cr_ifx.execute(f"""
        select
            nvl(cota_endereco.nom_bairro,endereco_correspondencia.nom_bairro) as nom_bairro,
            nvl(cota_endereco.nro_cep, endereco_correspondencia.nro_cep) as nro_cep,
            cpf_associado,
            nro_celular[1,20] as nro_celular,
            nvl(cota_endereco.nom_cidade, endereco_correspondencia.nom_cidade) as nom_cidade,
            nvl(cota_endereco.des_complemento, endereco_correspondencia.des_complemento) as des_complemento,
            dat_exp_identidade,
            dat_falecimento,
            dat_nascimento,
            des_email,
            cnpj_associado,
            (select des_cidade from minas:cidade as cidade where cidade.cod_uf = associado.cod_uf_nascimento and cidade.cod_cidade = associado.cod_cidade_nasc) as nom_cidade_natal,
            ...
            cod_profissao,
            tip_identidade,
            (select uf.cod_uf from uf where uf.cod_uf = nvl(cota_endereco.cod_uf, endereco_correspondencia.cod_uf)) as cod_uf,
            ...
            nvl(cota_endereco.des_logradouro, endereco_correspondencia.des_logradouro) as des_logradouro,
            nom_associado,
            nom_cargo_exerce,
            nom_empresa_trab,
            nom_mae,
            nom_pai,
            nro_identidade,
            nvl(cota_endereco.nro_endereco, endereco_correspondencia.nro_endereco) as nro_endereco,
            org_emissor,
            nvl(cota_endereco.nom_pais, endereco_correspondencia.nom_pais) as nom_pais,
        ...
        left join {linha_log.banco}:cota_endereco as cota_endereco on 
            cota_endereco.cod_titular = associado.cod_associado and
            cota_endereco.cod_tipo_associado = associado.cod_tipo_prior and
            cota_endereco.cod_cota = associado.cod_cota_prior
        where
