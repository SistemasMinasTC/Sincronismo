...
def createTriggersInformix(banco: str, tabela: str, primaryKey: list):
    ...
    cr.execute(f"""
        ...
        for each row
            ...
            (
                insert into mc_log(data_hora,tabela,operacao,pk,tentativas)
                values
                (
                    current,
                    '{tabela}',
...
def createTriggersMssql(tabela: str, primaryKey):
    sql = conecta_mssql()
    ...
    cr.execute(f"""
        ...
        for insert, update, delete as
            if object_id('tempdb..#sincronizando') is null
                begin
                    set nocount on;
                    insert into mc_log(data_hora,tabela,operacao,pk,excluido)
                    select
                        getdate(),
                        '{tabela}',
                        'del',
                    ...
                    from deleted
                    where
                        deleted.{primaryKey} not in (select inserted.{primaryKey} from inserted)
                    union all
                    select
                        getdate(),
                        '{tabela}',
    ...
    cr.execute(f"""
        create or alter trigger UltimaAlteracao{tabela} on {tabela}
            for update as
                update {tabela} set UltimaAlteracao = getdate() where {primaryKey} in (select inserted.{primaryKey} from inserted)
        """)
...
if __name__ == "__main__":
    try:
        createTriggersMssql('LogAcessoClube','IdMovimento')
