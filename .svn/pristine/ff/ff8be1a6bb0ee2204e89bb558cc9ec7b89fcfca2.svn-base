...
def convert(conn_ifx, conn_sql, linha_log):
    ...
    cr_ifx.execute(f"""
        select
            des_tipo_documento
        from {linha_log.banco}:tipo_documento as tipo_documento
        where
            idf_tipo_documento = ?
    """,(
    ...
    if cr_sql.rowcount == 0:
        ...
        cr_sql.execute(f"""
            ...
            (
                Nome
            ) output inserted.IdTipoDocumento  values (
                ? /*Nome*/
            )
        """,(
            origem.des_tipo_documento
        ...
        cr_sql.execute("insert into PkDePara values ('TipoDocumento',?,?)",(pkSql, linha_log.pk,))
